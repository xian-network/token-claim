import type { IXianWalletUtils } from "./xian-dapp-interface";

const XianWalletUtils: IXianWalletUtils = {
        ...
        document.addEventListener("xianWalletTxStatus", (event: any) => {
            ...
            if (this.state.transaction.requests.length > 0) {
                ...
                if ("errors" in event.detail) {
                    ...
                    this.getTxResultsAsyncBackoff(event.detail.txid)
                        .then((tx) => {
                            ...
                            parsedData.original_tx = JSON.parse(
                                this.hexToString(decodedOriginalTx),
                            );
                            resolver(parsedData);
                        })
                        .catch((error) => {
        ...
        return new Promise((resolve) => {
            if (this.isWalletReady) {
                ...
                setTimeout(() => {
                    if (!this.isWalletReady) {
                        const index =
                            this.state.walletReady.resolvers.indexOf(resolve);
                        if (index !== -1) {
        ...
        return new Promise((resolve, reject) => {
            ...
            const timeoutId = setTimeout(() => {
                ...
                if (index !== -1) {
                    ...
                    reject(
                        new Error(
                            "Xian Wallet Chrome extension not installed or not responding",
                        ),
                    );
                }
            }, 2000); // 2 seconds timeout
        ...
        return new Promise((resolve, reject) => {
            ...
            const timeoutId = setTimeout(() => {
                ...
                if (index !== -1) {
                    ...
                    reject(
                        new Error(
                            "Xian Wallet Chrome extension not responding",
                        ),
                    );
                }
            ...
            document.dispatchEvent(
                new CustomEvent("xianWalletSignMsg", {
                    detail: {
                        message: message,
                    },
                }),
    ...
    sendTransaction: async function (contract, method, kwargs) {
        ...
        return new Promise((resolve, reject) => {
            ...
            document.dispatchEvent(
                new CustomEvent("xianWalletSendTx", {
                    detail: {
                        contract: contract,
                        method: method,
                        kwargs: kwargs,
    ...
    getBalanceRequest: async function (address, contract) {
        const response = await fetch(
            `${this.rpcUrl}/abci_query?path=%22/get/${contract}.balances:${address}%22`,
        );
        if (!response.ok) {
            throw new Error("Network response was not ok");
        }
    ...
    getApprovedBalanceRequest: async function (
        token_contract,
        address,
        approved_to,
    ) {
        const response = await fetch(
            `${this.rpcUrl}/abci_query?path=%22/get/${token_contract}.balances:${address}:${approved_to}%22`,
        );
        if (!response.ok) {
            throw new Error("Network response was not ok");
        }
    ...
    getApprovedBalance: async function (token_contract, approved_to) {
        const info = await this.requestWalletInfo();
        const address = info.address;
        const balance = await this.getApprovedBalanceRequest(
            token_contract,
            address,
            approved_to,
        );
    ...
    getTxResultsAsyncBackoff: async function (
        txHash,
        retries = 5,
        delay = 1000,
    ) {
        try {
            ...
            return await this.getTxResultsAsyncBackoff(
                txHash,
                retries - 1,
                delay * 2,
            );
